...
def train_videosumm(args, split, split_idx):
    fc_text_to_video = TextToVideoConv1d(input_dim=768, hidden_dim=512, output_dim=1024).to(args.device)
    ...
    parameters = [p for p in model.parameters() if p.requires_grad] + \
                 ...
                 [p for p in fc_text_to_video.parameters() if p.requires_grad]
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        model.train()
        stats = AverageMeter('loss', 'l2_loss', 'cls_loss', 'loc_loss', 'ctr_loss', 'inter_contrastive_loss', 'intra_contrastive_loss')
        ...
        for k, (video_list, text_list, mask_video_list, mask_text_list,
            ...
            new_video_feature = fc_text_to_video(text) # 4, n, 1024

            segmented_video_prime = torch.zeros_like(video) # 4, 407, 1024

            for i in range(new_video_feature.size(0)):  # new_video_feature의 개수(segment 개수만큼)
                idx = 0
                for k in range(len(n_frame_per_seg_list[i])):  # 배열의 길이만큼 반복
                    for j in range(n_frame_per_seg_list[i][k]):  # 각 배열 안의 정수 개수만큼 반복
                        if idx < segmented_video_prime.size(1):
                            segmented_video_prime[i][idx] += new_video_feature[i][k]
                            idx += 1
                        else:
                            break


            video_final = (segmented_video_prime + video)/2

            l2_loss = calc_l2_loss(video, video_final)
            # 전체 손실에 L2 손실 포함
            loss = cls_loss + loc_loss + ctr_loss + inter_contrastive_loss + intra_contrastive_loss + l2_loss
            ...
            if (k + 1) % args.print_freq == 0:
                logger.info(f'[Train] Epoch: {epoch+1}/{args.max_epoch} Iter: {k+1}/{data_length} '
                            ...
                            f'Loss: {stats.loss:.4f} (l2: {stats.l2_loss:.4f}, cls: {stats.cls_loss:.4f}, '
                            f'loc: {stats.loc_loss:.4f}, ctr: {stats.ctr_loss:.4f}, '
                            f'inter_contrastive: {stats.inter_contrastive_loss:.4f}, '
                            f'intra_contrastive: {stats.intra_contrastive_loss:.4f})')
        ...
        args.writer.add_scalar(f'Split{split_idx}/Train/l2_loss', stats.l2_loss, epoch+1)

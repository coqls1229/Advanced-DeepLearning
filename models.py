...
class TextToVideoFC(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(TextToVideoFC, self).__init__()
...
class TextToVideoConv1d(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(TextToVideoConv1d, self).__init__()
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)  # Change back to [batch_size, seq_len, output_dim]
        return x
...
class Model_VideoSumm(nn.Module):
    def __init__(self, args=None):
        ...
        self.type_video = nn.Parameter(torch.zeros(1, 1, num_hidden)) #이건 뭐?
        ...
        self.fc_video_loc = nn.Linear(num_hidden, 2) # location?
        self.fc_video_ctr = nn.Linear(num_hidden, 1) #ctr?????
    ...
    def select_contrastive_embedding(self, score, embedding, mask, label):
        ...
        for i in range(B):
            ...
            key_embedding_num = max(1, torch.div(length, self.ratio, rounding_mode='trunc'))
            nonkey_embedding_num = max(1, torch.div(length, self.ratio, rounding_mode='trunc'))
    ...
    def forward(self, **kwargs):
        ...
        # prepend(맨 앞) the [CLSV] and [CLST] tokens to the video and text feature sequences
        ...
        video = video + self.pos_embed_video[:, :N_video, :] + self.type_video # type_video를 왜 더하지
        ...
        # fc layer를 통해 new_video feature 생성
        # new_video_feature = self.fc_text_to_video(text)
